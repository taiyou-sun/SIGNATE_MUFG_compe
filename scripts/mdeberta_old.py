...
train_ids, validation_ids, train_features, validation_features,train_masks,validation_masks, train_labels, validation_labels = train_test_split(input_ids, features,attention_masks, labels, random_state=123, test_size=0.1)
...
class AutoModelMultimodal(nn.Module):
    ...
    def forward(self, input_ids, additional_features, attention_mask, token_type_ids, labels=None):
        ...
        # 融合された特徴量を用いて分類
        logits = self.classifier(automodel_output)
...
# 使用例
model = AutoModelMultimodal(num_labels=5, num_features=3)
model.cuda()
...
for _ in trange(epoch, desc="Epoch"):
  ...
  if validation_score > max_score:
    save_file = 'pths/mDeberta_qwk_old.pth'
...
model.load_state_dict(torch.load('pths/mDeberta_qwk_old.pth'))
model.to(device)
...
save_df.to_csv('output_mDeberta_qwk_old.csv', index=False, header=False) 