...
tokenizer = AutoTokenizer.from_pretrained('microsoft/deberta-v3-base', do_lower_case=True)
...
class AutoModelMultimodal(nn.Module):
    def __init__(self, num_labels, num_features, dropout_rate=0.1):
        super().__init__()
        self.automodel = AutoModel.from_pretrained('microsoft/deberta-v3-base')
...
for _ in trange(epoch, desc="Epoch"):
  ...
  if validation_score > max_score:
    save_file = 'deberta_qwk_best.pth'
...
model.load_state_dict(torch.load('deberta_qwk_best.pth'))
model.to(device)
...
save_df.to_csv('output_deberta_qwk_best.csv', index=False, header=False) 