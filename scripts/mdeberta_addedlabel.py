...
class AutoModelMultimodal(nn.Module):
    def __init__(self, num_labels, num_features, dropout_rate=0.1):
        ...
        self.compress_automodel = nn.Linear(self.automodel.config.hidden_size, 10)
        
        self.regressor = nn.Sequential(
            nn.Linear(10+num_features, 10+num_features),
            nn.LayerNorm(10+num_features),
            nn.GELU(),
            nn.Linear(10+num_features, num_labels),
            nn.Softmax(dim=1)
        )
        self.num_labels = num_labels

    def forward(self, input_ids, additional_features, attention_mask, token_type_ids):
        ...
        # 追加特徴量を加算
        pooled_automodel_output, _ = automodel_output['last_hidden_state'].max(1)
        compressed_output = self.compress_automodel(pooled_automodel_output)
        fused_output = torch.cat([compressed_output, feature_encoded], dim=1)
        
        # 融合された特徴量を用いて分類
        logits = self.regressor(fused_output)
...
# CSV読み込み
df_all = pd.read_csv('openai/added_train.csv')
...
features = df[['thumbsUpCount', 'reviewCreatedVersion', 'timeToReply', 'PN']].values
labels = df.score.values

loss_weights = torch.tensor([0.6, 1.0, 1.0, 0.8, 0.4]).to(device).float()
print(loss_weights)
...
# 使用例
num_labels = 5
model = AutoModelMultimodal(num_labels=num_labels, num_features=4)
model.cuda()
# # 入力は前のアプローチと同様
...
for _ in trange(epoch, desc="Epoch"):
  ...
  if validation_score > max_score:
    save_file = 'pths/mDeberta_feture_score.pth'
...
model.load_state_dict(torch.load('pths/mDeberta_feture_score.pth'))
model.to(device)

df_test = pd.read_csv('nlp/added_test.csv')
...
sentences = ["[CLS] " + reviews[i] + " [SEP] " + replies[i] for i in range(len(reviews))]
features = df_test[['thumbsUpCount', 'reviewCreatedVersion', 'timeToReply', 'score']].values
...
save_df.to_csv('output_mDeberta_feture_score.csv', index=False, header=False) 