...
def get_dataloader(input_ids,attention_mask, features, labels, batch_size):
...
def to_SVC_input(df):
    ...
    df['text'] = df['text'].apply(lambda x: split_into_words(x))

    return df


def train_model(model, train_dataloader, validation_dataloader, epoch=10):
    model.train()
...
train_df, validation_df, train_labels, validation_labels = train_test_split(train_df, train_df.score.values, random_state=123, test_size=0.00000001)
...
print("Training TF-IDF+SVC model...")
df_train = to_SVC_input(train_df)
df_validation = to_SVC_input(validation_df)
X_train = [' '.join(row) for row in df_train['text'].values]
X_validation = [' '.join(row) for row in df_validation['text'].values]
# tf-idf
...
print("svc check")
# output
y_pred = svm_model.predict(X_validation_tfidf)
SVC_prediction = torch.tensor(y_pred).to(device)
num_classes = 5  # クラス数（ここでは0から4の5クラス）
SVC_prediction = F.one_hot(SVC_prediction, num_classes=num_classes).to(torch.float32)
...
validation_dataloader = get_dataloader(validation_inputs["input_ids"], validation_inputs["attention_mask"],validation_features, torch.tensor(validation_labels), batch_size=batch_size)
distilbert_predictions = train_model(distilbert_model, train_dataloader, validation_dataloader)
torch.save(distilbert_model.state_dict(), 'distilbert_ensamble_features_SVC.pth')
...
train_dataloader = get_dataloader(train_inputs["input_ids"], train_inputs["attention_mask"], train_features, torch.tensor(train_labels), batch_size=batch_size)
validation_dataloader = get_dataloader(validation_inputs["input_ids"], validation_inputs["attention_mask"], validation_features, torch.tensor(validation_labels), batch_size=batch_size)
xlnet_predictions = train_model(xlnet_model, train_dataloader, validation_dataloader)
torch.save(xlnet_model.state_dict(), 'xlnet_ensamble_features_SVC.pth')
print("Model saved successfully.")
...
train_dataloader = get_dataloader(train_inputs["input_ids"], train_inputs["attention_mask"], train_features, torch.tensor(train_labels), batch_size=batch_size)
validation_dataloader = get_dataloader(validation_inputs["input_ids"], validation_inputs["attention_mask"], validation_features, torch.tensor(validation_labels), batch_size=batch_size)
roberta_predictions = train_model(roberta_model, train_dataloader, validation_dataloader)
torch.save(roberta_model.state_dict(), 'roberta_ensamble_features_SVC.pth')
print("Model saved successfully.")
...
SVC_prediction = SVC_prediction.to(device)

ensemble_prediction = (distilbert_predictions[-1] + xlnet_predictions[-1] + roberta_predictions[-1] + SVC_prediction) / 4
print(ensemble_prediction)
print(f"Validation Accuracy: {flat_accuracy(ensemble_prediction, torch.tensor(validation_labels).to(device)):.4f}")

# labels = torch.tensor(validation_labels).to(device)
# print(labels.shape)
# for epoch, prediction in enumerate(ensemble_predictions):
#     print(prediction.shape)
#     print(f"Epoch {epoch + 1} Validation Accuracy: {flat_accuracy(prediction, labels):.4f}")