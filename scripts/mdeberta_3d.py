...
# 使用例
model = AutoModelMultimodal(num_labels=3, num_features=3)
model.cuda()
# # 入力は前のアプローチと同様
...
for _ in trange(epoch, desc="Epoch"):
  ...
  if validation_score > max_score:
    save_file = 'pths/mDeberta_qwk_best_3d.pth'
...
model.load_state_dict(torch.load('pths/mDeberta_qwk_best_3d.pth'))
model.to(device)
...
save_df.to_csv('output_mDeberta_qwk_best_3d.csv', index=False, header=False) 