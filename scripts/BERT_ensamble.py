...
import warnings
from transformers import AutoTokenizer, AutoModel, XLNetTokenizer, XLNetModel, RobertaTokenizer, RobertaModel, AdamW
...
class MultiModel(nn.Module):
    def __init__(self, model, hidden_size, num_features):
...
def train_model(model, train_dataloader, validation_dataloader, label_num, epoch=10):
    model.train()
    ...
    for _ in trange(epoch, desc="Epoch"):
        ...
        for batch in validation_dataloader:
            ...
            predictions = torch.round(outputs).clamp(0, label_num - 1).int()
...
mdeberta_tokenizer = AutoTokenizer.from_pretrained('microsoft/deberta-v3-base')
mdeberta_model = AutoModel.from_pretrained('microsoft/deberta-v3-base')
...
# Initialize models
mdeberta_model = MultiModel(mdeberta_model, hidden_size=768, num_features=3)
xlnet_model = MultiModel(XLNetModel.from_pretrained('xlnet-base-cased'), hidden_size=768, num_features=3)
roberta_model = MultiModel(RobertaModel.from_pretrained('roberta-base'), hidden_size=768, num_features=3)
...
mdeberta_predictions = train_model(mdeberta_model, train_dataloader, validation_dataloader, label_num=5)
torch.save(mdeberta_model.state_dict(), 'mdeberta_ensamble_features_tooweak.pth')
...
train_dataloader = get_dataloader(train_inputs["input_ids"], train_inputs["attention_mask"], train_features, torch.tensor(train_labels), batch_size=batch_size)
validation_dataloader = get_dataloader(validation_inputs["input_ids"], validation_inputs["attention_mask"], validation_features, torch.tensor(validation_labels), batch_size=batch_size, shuffle=False)
roberta_predictions = train_model(roberta_model, train_dataloader, validation_dataloader, label_num=5)
torch.save(roberta_model.state_dict(), 'roberta_ensamble_feature_tooweak.pth')
print("Model saved successfully.")
...
train_dataloader = get_dataloader(train_inputs["input_ids"], train_inputs["attention_mask"], train_features, torch.tensor(train_labels), batch_size=batch_size)
validation_dataloader = get_dataloader(validation_inputs["input_ids"], validation_inputs["attention_mask"], validation_features, torch.tensor(validation_labels), batch_size=batch_size, shuffle=False)
xlnet_predictions = train_model(xlnet_model, train_dataloader, validation_dataloader, label_num=5)
torch.save(xlnet_model.state_dict(), 'xlnet_ensamble_features_tooweak.pth')
print("Model saved successfully.")
...
ensemble_predictions = (mdeberta_predictions + xlnet_predictions + roberta_predictions) / 3
