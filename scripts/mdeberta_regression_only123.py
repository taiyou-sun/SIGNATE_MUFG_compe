...
class AutoModelMultimodal(nn.Module):
    def __init__(self, num_labels, num_features, dropout_rate=0.0):
        super().__init__()
...
for fold, (train_index, val_index) in enumerate(kf.split(original_ids)):
    ...
    validation_features = torch.tensor(validation_features).float()
    train_labels = torch.concat([torch.tensor(train_labels).float(), add_labels.float()])
    validation_labels = torch.tensor(validation_labels)
    ...
    for _ in trange(epoch, desc="Epoch"):
        ...
        for batch in validation_dataloader:
            ...
            # b_input_ids.size(0) / batch_size　は、最後のバッチのサイズを考慮するためのもの
        ...
        if validation_score > max_score:
            save_dir = f'pths/mDeberta_qwk_regression_oversampled'
