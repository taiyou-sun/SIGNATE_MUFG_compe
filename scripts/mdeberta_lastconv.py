...
class AutoModelMultimodal(nn.Module):
    def __init__(self, num_labels, num_features, dropout_rate=0.1):
        ...
        self.automodel.gradient_checkpointing_enable()
        self.automodel.base_model.embeddings.requires_grad_(False)
        ...
        self.cnn1 = nn.Conv1d(self.automodel.config.hidden_size, 256, kernel_size=2, padding=1)
        self.cnn2 = nn.Conv1d(256, num_labels, kernel_size=2, padding=1)
    ...
    def forward(self, input_ids, additional_features, attention_mask, token_type_ids):
        ...
        # 追加特徴量を加算
        last_hidden_state = automodel_output['last_hidden_state'].permute(0, 2, 1)
        #fused_output = torch.cat([last_hidden_state, feature_encoded], dim=1)
        cnn_embeddings = F.relu(self.cnn1(last_hidden_state))
        cnn_embeddings = self.cnn2(cnn_embeddings)
        padded_output, _ = torch.max(cnn_embeddings, 2)
        logits = self.softmax(padded_output)
...
# CSV読み込み
df_all = pd.read_csv('nlp/added_train.csv')
...
for _ in trange(epoch, desc="Epoch"):
  ...
  if validation_score > max_score:
    save_file = 'pths/mDeberta_qwk_lastconv.pth'
...
model.load_state_dict(torch.load('pths/mDeberta_qwk_lastconv.pth'))
model.to(device)
...
save_df.to_csv('output_mDeberta_qwk_lastconv.csv', index=False, header=False) 