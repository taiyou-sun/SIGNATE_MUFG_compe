...
def outputcheck_model(model, train_dataloader):
    model.eval()
...
# Load the models
# distilbert_model.load_state_dict(torch.load('distilbert_ensamble_features.pth'))
# distilbert_model.to(device)
xlnet_model.load_state_dict(torch.load('xlnet_ensamble_features.pth'))
xlnet_model.to(device)
# roberta_model.load_state_dict(torch.load('roberta_ensamble_features.pth'))
...
# train_dataloader = get_dataloader(train_inputs["input_ids"], train_inputs["attention_mask"],train_features, batch_size=batch_size)
...
prediction_labels = torch.argmax(xlnet_predictions, axis=1).detach().cpu().numpy()
