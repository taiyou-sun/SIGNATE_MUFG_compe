...
class AutoModelMultimodal(nn.Module):
    def __init__(self, num_labels, num_features, dropout_rate=0.1):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(self.automodel.config.hidden_size+num_features, 1)
        )
        self.num_labels = num_labels
        self.thresholds = nn.Parameter(torch.randn(num_labels - 1))  # Initialize thresholds with random values
    ...
    def forward(self, input_ids, additional_features, attention_mask, token_type_ids, labels=None):
        ...
        thresholds = torch.cumsum(torch.sigmoid(self.thresholds), dim=0)  # Ensure thresholds are ordered

        return logits.squeeze(1), thresholds
        
    def predict(self, input_ids, additional_features, attention_mask, token_type_ids):
        self.eval()  # 推論モードに設定
        with torch.no_grad():  # 勾配計算をオフに
            logits, threshold = self.forward(input_ids, additional_features, attention_mask, token_type_ids)
        predictions = categorize_labels(logits, threshold)
...
def categorize_labels(logits, thresholds):
    # ロジットを閾値と比較して、予測クラスを決定
    thresholds = torch.cat([torch.tensor([-float('inf')]).to(thresholds.device), thresholds, torch.tensor([float('inf')]).to(thresholds.device)])
    
    # 各サンプルについて、logitがどの閾値を超えたかを判定
    pred_classes = torch.sum(logits.unsqueeze(1) > thresholds, dim=1) - 1
    return pred_classes
...
class OrdinalLoss(nn.Module):
    ...
    def forward(self, logits, thresholds, targets):
        logits = logits.unsqueeze(1)
        thresholds = thresholds.unsqueeze(0)
        
        cumulative_probs = torch.sigmoid(logits - thresholds)

        loss = 0
        for i in range(thresholds.shape[1]):
            target_class = (targets > i).float()
            loss_per_class = nn.BCELoss()(cumulative_probs[:, i], target_class)
...
df_all = limit_scorecounts(df_all, 4, 2000)
# df_all = limit_scorecounts(df_all, 0, 1000)
...
for fold, (train_index, val_index) in enumerate(kf.split(input_ids)):
    ...
    for _ in trange(epoch, desc="Epoch"):
        ...
        criterion = OrdinalLoss(loss_weights)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            predictions, threshold = model(b_input_ids, b_input_features, token_type_ids=None, attention_mask=b_input_mask)
            loss = criterion(predictions, threshold, b_labels)
            loss.backward()
            optimizer.step()
            tr_loss += loss.item()
        ...
        for batch in validation_dataloader:
            ...
            with torch.no_grad():
                logits, thresholds = model(b_input_ids, b_input_features, token_type_ids=None, attention_mask=b_input_mask)

            predictions = categorize_labels(logits, thresholds)
            predictions = predictions.detach().cpu().numpy()
            label_ids = b_labels.to('cpu').numpy()
            # prediction_data = np.concatenate((b_row_ids.reshape(-1, 1),label_ids.reshape(-1, 1), predictions.reshape(-1, 1)), axis=1)
            # prediction_fail = prediction_data[prediction_data[:,1] != prediction_data[:,2]]
        ...
        if validation_score > max_score:
            save_dir = f'pths/mDeberta_qwk_ordinalregression_downsampled'
