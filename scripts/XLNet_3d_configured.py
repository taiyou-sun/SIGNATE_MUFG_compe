...
print("XLNetのみ（feature読み込みなど全くなし）")
# CSV読み込み
...
class XLNetMultimodal(nn.Module):
    ...
    def forward(self, input_ids, additional_features, attention_mask, token_type_ids, labels=None):
        ...
        # 融合された特徴量を用いて分類
        logits = self.classifier(xlnet_output[:,0])
...
torch.save(model.state_dict(), 'XLNet_3d_configured.pth')
print("Model saved successfully.")

# testの実行

model.load_state_dict(torch.load('XLNet_3d_congigured.pth'))
model.to(device)
...
save_df.to_csv('output_3d_configured.csv', index=False) 